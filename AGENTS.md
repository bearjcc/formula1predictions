...
**First admin (deploy):** After deploy, promote your user: set `ADMIN_EMAIL` in .env and run `php artisan app:promote-admin`, or run `php artisan app:promote-admin you@example.com`. The user must already exist (e.g. register first).
