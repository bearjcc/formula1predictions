...
class LegacyCsvPredictions2022Seeder extends Seeder
{
    /**
    ...
    {
        if ($this->command) {
            $this->command->info('Importing 2022 predictions from CSV (storage/app/xlsx_export/predictions_2022.csv)...');
        }

        $path = storage_path('app/xlsx_export/predictions_2022.csv');
        ...
        try {
            ...
            while (($row = fgetcsv($handle)) !== false) {
                ...
                if ($predictor === '' || ! isset($users[$predictor])) {
                    // Skip predictors we are not mapping into seed data
                    continue;
                }
        ...
        foreach ($grouped as $predictor => $races) {
            ...
            foreach ($races as $raceName => $positions) {
                ...
                $this->createRacePrediction($user, 2022, $raceName, $driverOrder);
            }
        }

        if ($this->command) {
            $this->command->info('2022 CSV predictions imported successfully.');
        }
    }
    ...
    private function normalizeDriverName(string $name): ?string
    {
        ...
        // Common name mappings (keep in sync with HistoricalPredictionsSeeder)
        $mappings = [
            ...
            'Sebastain Vettel' => 'Sebastian Vettel',
        ];
    ...
    {
        // Map race names to their typical round numbers (keep in sync with HistoricalPredictionsSeeder)
        $raceRounds = [
            'Bahrain' => 1,
            'Saudi Arabia' => 2,
            'Australia' => 3,
