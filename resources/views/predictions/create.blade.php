...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-6">Create New Prediction</h1>
        ...
        <form action="{{ route('predictions.store') }}" method="POST" class="space-y-6">
            ...
            <div class="form-control">
                ...
                <input type="number" name="season" id="season" class="input input-bordered" 
                       value="{{ old('season', 2024) }}" min="2022" max="2024" required>
                @error('season')
                    <div class="text-error text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>

            <div class="form-control" id="race-round-field" style="display: none;">
                ...
                <input type="number" name="race_round" id="race_round" class="input input-bordered" 
                       value="{{ old('race_round') }}" min="1" max="24">
                @error('race_round')
                    <div class="text-error text-sm mt-1">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('predictions.index') }}" class="btn btn-outline">Cancel</a>
            </div>
        </form>
    </div>
</div>
