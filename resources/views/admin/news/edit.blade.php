<x-layouts.layout title="Admin – Edit post" headerSubtitle="Edit news or announcement">
            ...
            <form action="{{ route('admin.news.update', $news) }}" method="POST">
                ...
                <div class="form-control w-full mb-4">
                    ...
                    <textarea id="excerpt" name="excerpt" rows="2" maxlength="500"
                        ...
                        placeholder="Short summary for listings and RSS">{{ old('excerpt', $news->excerpt) }}</textarea>
                    @error('excerpt')
                        <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                <div class="form-control w-full mb-4">
                    ...
                    <textarea id="body" name="body" rows="12" required
                        ...
                        placeholder="Full content (plain text or HTML)">{{ old('body', $news->body) }}</textarea>
                    @error('body')
                        <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
