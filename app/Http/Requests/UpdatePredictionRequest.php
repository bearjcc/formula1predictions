...
class UpdatePredictionRequest extends FormRequest
{
    ...
    {
        return [
            'type' => ['sometimes', 'required', 'string', Rule::in(['race', 'sprint', 'preseason', 'midseason'])],
            'season' => ['sometimes', 'required', 'integer', 'min:1950', 'max:2030'],
            ...
            'prediction_data.driver_order' => ['required_if:type,race,sprint', 'array', 'min:1', 'max:20'],
            ...
            'prediction_data.team_order' => ['required_if:type,preseason,midseason', 'array', 'min:1', 'max:10'],
            'prediction_data.team_order.*' => ['required_if:type,preseason,midseason', 'integer', 'exists:teams,id'],
            'prediction_data.driver_championship' => ['required_if:type,preseason,midseason', 'array', 'min:1', 'max:20'],
    ...
    public function messages(): array
    {
        return [
            ...
            'prediction_data.driver_order.min' => 'Driver order must include between 1 and 20 drivers.',
            'prediction_data.driver_order.max' => 'Driver order must include between 1 and 20 drivers.',
            'prediction_data.team_order.required_if' => 'Team order is required for preseason and midseason predictions.',
            'prediction_data.team_order.min' => 'Team order must include between 1 and 10 teams.',
            'prediction_data.team_order.max' => 'Team order must include between 1 and 10 teams.',
            ...
            'prediction_data.driver_championship.min' => 'Driver championship order must include between 1 and 20 drivers.',
            'prediction_data.driver_championship.max' => 'Driver championship order must include between 1 and 20 drivers.',
